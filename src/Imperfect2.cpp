...
struct Imperfect2 : core::AHModule {
	...
	Imperfect2() : core::AHModule(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS) {
		...
		for (int i = 0; i < 4; i++) {
			...
			params[DELAYSPREAD_PARAM + i].config(1.0f, 2.0f, 1.0f, "Delay length spread", "ms", -2.0f, 2000.0f, 0.0f);
			...
			params[LENGTHSPREAD_PARAM + i].config(1.0, 2.0, 1.0f, "Gate length spread", "ms", -2.0f, 2000.0f, 0.0f);
